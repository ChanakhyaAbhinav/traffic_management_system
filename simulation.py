...
#     Other simulation logic remains unchanged.
import random
import math
import time
import threading
...
speeds = {'car':2.5, 'bus':2, 'truck':1.9, 'rickshaw':2, 'bike':2.5} 
...
# Default probabilities for vehicle generation
traffic_probabilities = {
    "Lane 1": 0.5,  # Default probability
    "Lane 2": 0.5,
    "Lane 3": 0.5,
    "Lane 4": 0.5,
}

# Read traffic conditions from the file
if os.path.exists("traffic_conditions.txt"):
    with open("traffic_conditions.txt", "r") as f:
        for line in f:
            lane, probability = line.strip().split(":")
            traffic_probabilities[lane] = float(probability)

# Generating vehicles in the simulation based on probabilities
def generateVehicles():
    while True:
        for lane, probability in traffic_probabilities.items():
            
            if random.random() < probability:  # Generate a vehicle based on the probability
                vehicle_type = random.randint(0, 4)
                lane_number = random.randint(0, 2)
                will_turn = random.choice([0, 1])
                direction_number = list(traffic_probabilities.keys()).index(lane)  # Map lane to direction
                Vehicle(lane_number, vehicleTypes[vehicle_type], direction_number, directionNumbers[direction_number], will_turn)

                # Increment the vehicle count for the lane
                vehicle_counts[lane] += 1
        print("Vehicles generated", vehicle_counts)
        time.sleep(0.75)
...
class Main:
    ...
    with open(traffic_data_path, "w") as f:
        json.dump([], f)  # Write an empty list to clear the file
    print("Cleared traffic_data.json.")
    
    save_timer = 0  # Timer to track 5-second intervals
    ...
    while True:
        ...
        save_timer += 1
        if save_timer >= 300:  # 300 iterations of the loop (assuming ~60 FPS) = 5 seconds
            traffic_data = []
            for i in range(noOfSignals):
                lane = directionNumbers[i]
                crossed_vehicles = vehicles[lane]['crossed']
                data = {
                    "time": datetime.now().strftime("%H:%M:%S"),  # Current time in hr:min:sec format
                    "lane": lane,
                    "vehicles_crossed": crossed_vehicles
                }
            ...
            try:
                ...
                # Append the new data
                if isinstance(existing_data, list):  # Ensure the file contains a list
                    existing_data.extend(traffic_data)
                else:
                    existing_data = traffic_data

                # Write the updated data back to the file
                with open(traffic_data_path, "w") as f:
                    json.dump(existing_data, f, indent=4)

                print("Vehicle counts saved to traffic_data.json.")
            except Exception as e:
                print(f"Failed to save data to traffic_data.json: {e}")
